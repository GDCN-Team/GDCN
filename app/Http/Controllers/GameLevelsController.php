...
class GameLevelsController extends Controller
{
    ...
    /**
     * GameLevelsController constructor.
     ...
     */
    public function search(GameLevelSearchRequest $request, Helpers $helper, GameHashesController $hash)
    {
        try {
            $data = $request->validated();
            ...
            if (!empty($data['featured'])) {
                $data['type'] = GameLevelSearchType::FEATURED;
            }
            ...
            if (!empty($data['epic'])) {
                $data['type'] = GameLevelSearchType::HALL_OF_FAME;
            }
            ...
            switch ($data['type']) {
                case GameLevelSearchType::SEARCH:
                    is_numeric($data['str']) ? $query->find($data['str']) : $query->where('name', 'LIKE', '%' . $data['str'] . '%');
                    break;
                case GameLevelSearchType::MOST_DOWNLOADED:
                    $query->orderByDesc('downloads');
                    break;
                case GameLevelSearchType::MOST_LIKED:
                    $query->orderByDesc('likes');
                    break;
                case GameLevelSearchType::TRENDING:
                    $query->where('created_at', '>', Carbon::rawParse('last week'));
                    $query->orderByDesc('likes');
                    break;
                case GameLevelSearchType::RECENT:
                    $query->orderByDesc('created_at');
                    break;
                case GameLevelSearchType::USER:
                    $query->where('user', $data['str']);
                    break;
                case GameLevelSearchType::FEATURED:
                    $query->whereHas('rating', function (Builder $query) {
                        $query->where('featured_score', '>', 0);
                    });
                    break;
                case GameLevelSearchType::MAGIC:
                    $query->where('objects', '>', 9999);
                    break;
                case GameLevelSearchType::PACK:
                    $query->whereIn('id', $data['str']);
                    break;
                case GameLevelSearchType::AWARDED:
                    $query->whereHas('rating', function (Builder $query) {
                        $query->where('stars', '>', 0);
                    });
                    break;
                ...
                case GameLevelSearchType::FRIENDS:
                    ...
                    break;
                case GameLevelSearchType::HALL_OF_FAME:
                    $query->whereHas('rating', function (Builder $query) {
     ...
     */
    public function download(GameLevelDownloadRequest $request, GameHashesController $hash)
    {
        try {
            $request->validateChk();
     ...
     */
    public function getDaily(GameDailyLevelGetRequest $request)
    {
        $data = $request->validated();
    ...
    {
        $data = $request->validated();
        if ($log->existsOrNew(GameLogType::fromValue(GameLogType::REPORT_LEVEL), $data['levelID'], null, true)) {
